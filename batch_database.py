...
from Depertment_database import createDepartment
...
with open("Batch_database.csv", "a", newline = "\n") as f:
...
csv_reader[len(csv_reader) - 1][3] = csv_reader[len(csv_reader) - 1][3] + ":" +
course_id
...
with open("Depertment_database.csv", "r", newline = "\n") as f:
csv_reader = list(csv.reader(f, delimiter=","))
check = 0
for i in range(0, len(csv_reader)):
...
csv_reader[i][2] = csv_reader[i][2] + ":" + batch_id
df = pandas.read_csv("Depertment_database.csv")
df.loc[i-1, "list_of_batches"] = csv_reader[i][2]
df.to_csv("Depertment_database.csv", index = False)
if(check == 0):
